...
## Notes
- Enter airflow worker:
  - ```docker
    docker ps
    docker exec -it 'workerid' bash
    ```
- Airflow keeps records and logs of runs so sometimes it doesn't rerun tasks when start date is changed
  - inside airflow worker execute:
  - ```
    airflow dags backfill yellow_taxi_data_v2 --reset-dagruns -s 2021-01-01 -e 2022-01-01
    ```
    
- Google credentials are set up in docker compose as env variable
